...
class ppt extends React.Component {
    ...
    componentWillMount(){
      ...
      response.json().then((body) => {
        console.log(body);
        this.setState({ppt_path:"http://localhost:5000/return-files?pptpath="+body.ppt_path,src:"http://localhost:5000/return-files?pptpath="+body.pdf_path+"#page=1",hierarchy:body.subtopic_mapping,display:body.page_mapping[this.state.topic], page_mapping: body.page_mapping});
    ...
    print(index){
        ...
        console.log(index);
        this.setState({topic:index,index:this.state.index+1,display:this.state.page_mapping[index]})
        ...
        const TreeRecursive = ({ data }) => {
            ...
            return data.map((item,index) => {
              ...
              if (item["children"].length == 0 || this.check_children(item["children"])) {
                if(item["heading"][1]=="h"){
                return <MenuItem onClick={() => this.print(item["pg"])} style={{fontSize:"20px",color:"#f9f9f9d7"}}>{item["content"]}</MenuItem>;}
              }
              // if its a folder render <Folder />
              else {
                return (
                  <SubMenu title={item["content"]} onClick={() => this.print(item["pg"])}  style={{fontSize:"20px",color:"#f9f9f9d7"}}>
    ...
    if(this.state.page_mapping && this.state.hierarchy){
      console.log("*****************"+this.state.hierarchy)
      const Student=(<Link to={{pathname:"/student_assessments/", data:this.state.topic==Object.keys(this.state.page_mapping).length-1?Object.keys(this.state.page_mapping).length%2==0?((this.state.topic+1)/2|0)-1:((this.state.topic+1)/2|0):((this.state.topic+1)/2|0)-1, tid: this.state.id, topic:this.state.topic, topic_length:Object.keys(this.state.page_mapping).length}}>
          <button  className="btn-secondary btn-lg button2">Next &raquo;</button>
        </Link>);
  ...
  return (
	...
	<TreeRecursive data={this.state.hierarchy}/>
        ...
        <Content key={this.state.index} heading={this.state.page_mapping[this.state.topic]}></Content>
        <hr></hr>
        ...
        {jwt_decode(localStorage.usertoken).identity.role==="student" && ((this.state.topic+1)%2==0 || (this.state.topic==this.state.page_mapping.length-1))?(Student):
        <button onClick={() => this.state.topic+1<Object.keys(this.state.page_mapping).length?this.print(this.state.topic+1):null} className="btn-secondary btn-lg button2">Next &raquo;</button>}
        <br></br>
        <hr></hr>
        <br></br>
